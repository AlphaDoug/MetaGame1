...
export default class UIDefault extends UI.UIBehavior {
	...
	InteractBtn: UI.Button
	NearInteractGuid :string
	/* 解析资源ID列表 */
	...
	private TryInteract():void {
		let obj:GameObject = GameObject.find(this.NearInteractGuid)
		if (obj == null) {
			return
		}
		let a = obj.getScripts()
		let actor : InteractActor = <InteractActor>obj.getScriptByName("IInteraction")
		actor.OnInteract(getCurrentPlayer(), 1)
	}

	private ShowInteractButton(guid :string):void {
		this.InteractBtn.visibility = UI.SlateVisibility.Visible
		this.NearInteractGuid = guid
	}
	private HideInteractButton(guid:string):void{
    ...
    protected onStart() {
		...
		this.InteractBtn = this.uiWidgetBase.findChildByPath('RootCanvas/InteractBtn') as UI.Button
		this.InteractBtn.visibility = UI.SlateVisibility.Collapsed
		
		Events.addLocalListener("client_show_interact_button", (guid: string)=>{
			this.ShowInteractButton(guid)
		});	
		Events.addLocalListener("client_hide_interact_button", (guid: string)=>{
